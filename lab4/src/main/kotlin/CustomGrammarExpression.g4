grammar CustomGrammarExpression;
...
grammarNodeValue [GrammarData data]: SPEC_LEFT typeVal=INPUT VERT defVal=INPUT SPEC_RIGHT {
...
@init {
    RuleTarget target = new RuleTarget();
}: | VERT element[target]+ {$state.getTargets().add(target);} targetsPoint[$state];
...
terminalVariable [NonTerminalState state]:
    SPEC_LEFT VAR varName=NON_TERMINAL_NAME DOUBLE_POINT className=GRAMMAR_NAME SPEC_RIGHT {
